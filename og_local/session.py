...
class Session:
    ...
    def load(cls) -> "Session":
        path = session_path()
        if not path.exists():
            raise AuthError("not logged in — run `veil login` first")
        try:
    ...
    def access_token(self) -> str:
        ...
        if not token:
            raise AuthError("session has no access token — run `veil login` to sign in again")
        return token
    ...
    def _refresh(self) -> None:
        ...
        if not refresh_token:
            raise AuthError("session expired — run `veil login` to sign in again")
        ...
        if resp.status_code in (400, 401, 403):
            raise AuthError(
                ...
                "run `veil login` to sign in again"
            )
        try:
...
def login(app_url: str, *, open_browser: bool = True, timeout: float = 300.0) -> Session:
    ...
    try:
        ...
        print("Waiting for authorization… (Ctrl-C to cancel; or use `veil login --manual`)")
