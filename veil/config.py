...
# VEIL_APP_URL or `veil login --app-url ...`.
DEFAULT_APP_URL = os.getenv("VEIL_APP_URL", "https://chat.opengradient.ai")
...
def config_home() -> Path:
    ...
    ``VEIL_HOME`` (handy for tests and for running several identities).
    """
    override = os.getenv("VEIL_HOME")
...
class ServerConfig:
    ...
    def from_env(cls) -> "ServerConfig":
        return cls(
            host=os.getenv("VEIL_HOST", "127.0.0.1"),
            port=int(os.getenv("VEIL_PORT", "11434")),
            expected_pcr_hash=_norm_hex(os.getenv("VEIL_EXPECTED_PCR_HASH")),
            pinned_tee_id=_norm_hex(os.getenv("VEIL_TEE_ID")),
        )
